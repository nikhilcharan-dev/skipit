...
export default function Setup() {
  const { state, patch, resetInterview } = useStore();
  ...
  const [semDet,     setSemDet]     = useState([]);   // raw API det array
  ...
  async function fetchInterviewTypes() {
    try {
      const res = await apiPost(
        ...
        {
          user_cat_id:  state.userid?.user_cat_id,
          set_id:       1,
          mail:         state.email,
      ...
      // Response: { sts, interview_types, profile_id } — profile_id is top-level
  ...
  async function fetchSemesters() {
    ...
    try {
      ...
      // API returns { det: [{ Departments: {...}, semesters: [...] }, ...] }
      const det = res?.det || [];
      if (det.length > 0) {
        setSemDet(det);
        // initial dept is deptId (1001) — find matching entry
  ...
  // ── Fetch subjects whenever dept/sem changes (also called on step 1→2)
  ...
  // ── Single-select subject (close + clear search after pick)
  ...
  async function beginInterview() {
    ...
    const selSubs = selectedIds.map(id => {
        ...
        <div className="card" style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div className="form-group">
              ...
              >
                <option value="1001" data-s3="el_cse">Computer Science and IT</option>
                <option value="1002" data-s3="el_ece">Electronics &amp; Communication</option>
                <option value="1003" data-s3="el_mech">Mechanical Engineering</option>
              </select>
            </div>
